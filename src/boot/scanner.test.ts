...
describe("scanner", () => {
	...
	test("emits warn advisory when lock differs from provided packages", async () => {
		const original = process.env.BUN_OSV_ENABLE_STALE_LOCK_WARN;
		process.env.BUN_OSV_ENABLE_STALE_LOCK_WARN = "1";
		...
		const scanner = createScanner({
			...
			// Provide one synthetic advisory to satisfy the condition for appending stale warn
			securityService: createStubService(
				ok([
					{
						level: "warn",
		...
		// Expect synthetic advisory + stale warn appended
		expect(advisories).toEqual([
			{
				level: "warn",
				package: "A",
		...
		process.env.BUN_OSV_ENABLE_STALE_LOCK_WARN = original;
	});
